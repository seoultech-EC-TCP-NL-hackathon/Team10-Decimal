...
class Workspace(Base):
    ...
    subjects = relationship(
        "Subject", back_populates="workspace", cascade="all, delete-orphan"
    )
...
class Subject(Base):
    ...
    summary_jobs = relationship(
        "SummaryJob", back_populates="subject", cascade="all, delete-orphan"
    )
...
class SummaryJob(Base):
    ...
    source_materials = relationship(
        "SourceMaterial", back_populates="job", cascade="all, delete-orphan"
    )
    job_stage_logs = relationship(
        "JobStageLog", back_populates="job", cascade="all, delete-orphan"
    )
...
class SourceMaterial(Base):
    ...
    status = Column(
        SQLAlchemyEnum(MaterialStatus), nullable=False, default=MaterialStatus.UPLOADED
    )
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    ...
    speaker_attributed_segments = relationship(
...
class SpeakerAttributedSegment(Base):
    ...
    id = Column(Integer, primary_key=True)
    material_id = Column(
        Integer, ForeignKey("source_materials.id"), nullable=False, index=True
    )
    ...
    material = relationship(
        "SourceMaterial", back_populates="speaker_attributed_segments"
    )
